...
Request.prototype.end = self => {
	if (isEmpty(self._fields))
		return httpClient.request(
				self.method,
				self.url,
				self._data ? Platform.OS === 'ios' ? self._data : JSON.stringify(self._data) : null,
				self._header
			)

	return httpClient.multipartPostRequest(
		self.url,
		self._fields.fileURL,
		self._fields.name,
		self._fields.fileName,
		self._fields.mimeType,
		self._data ? Platform.OS === 'ios' ? self._data : JSON.stringify(self._data) : null,
		self._header
	)
}
...
Request.prototype.getHeader = (self, field) => self._header[field.toLowerCase()]
